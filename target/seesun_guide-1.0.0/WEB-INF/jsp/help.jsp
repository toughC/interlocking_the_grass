...
<div class="section">
	<h2 id="sqlquery">SQL 쿼리문</h2>
		<ul class="listStyle-01 row">
		<li>• eGovFrame에서 <strong>GIS</strong>를 활용하기 위한 자주 사용되는 SQL 쿼리문을 소개하겠습니다. 
		</li>
	</ul>
	<div class="info">
		<p>아래에서 확인이 가능합니다.</p>
		<div class="textBox">
			<ul>
				<li>1. <strong>레이어 조회</strong>: <code> SELECT * FROM 레이어_테이블명;</code>
				</br>위의 쿼리문은 특정 레이어 테이블에서 모든 레코드를 선택하여 조회하는 예시입니다. 레이어_테이블명은 실제로 사용하는 레이어 테이블의 이름으로 대체되어야 합니다.
				</li>
				</br>
				<li>2. <strong>공간 범위 내의 레이어 조회</strong>: <code>SELECT * FROM 레이어_테이블명 WHERE ST_Intersects(좌표열, ST_GeomFromText('POLYGON((경도1 위도1, 경도2 위도2, 경도3 위도3, 경도1 위도1))'));</code>
				</br>위의 쿼리문은 주어진 공간 범위 내에 속하는 레이어를 조회하는 예시입니다. 좌표열은 레이어 테이블에서 공간 정보를 나타내는 열의 이름으로 대체되어야 합니다. POLYGON 내에는 좌표값으로 이루어진 다각형을 정의하며, 검색하려는 공간 범위에 맞게 좌표값을 설정해야 합니다.
				</li>
				</br>
				<li>3. <strong>레이어의 속성 조회</strong>: <code>SELECT 속성1, 속성2 FROM 레이어_테이블명;</code>
				</br>위의 쿼리문은 특정 레이어 테이블에서 지정한 속성 열의 값을 조회하는 예시입니다. 속성1, 속성2는 실제로 사용하는 레이어 테이블의 속성 열의 이름으로 대체되어야 합니다.
				</li>
				</br>
				<li>4. <strong>공간 데이터의 거리 계산</strong>: <code>SELECT ST_Distance(좌표열1, 좌표열2) AS 거리 FROM 레이어_테이블명;</code>
				</br>위의 쿼리문은 두 공간 데이터 간의 거리를 계산하여 출력하는 예시입니다. 좌표열1, 좌표열2는 실제로 사용하는 레이어 테이블에서 공간 정보를 나타내는 열의 이름으로 대체되어야 합니다.
				</li>
				</br>
				<li>5. <strong>레이어 속성 필터링</strong>: <code>SELECT * FROM 레이어_테이블명 WHERE 속성 = '값';</code>
				</br>위의 쿼리문은 특정 속성 열에서 지정한 값과 일치하는 레코드를 조회하는 예시입니다. 레이어_테이블명은 실제 레이어 테이블의 이름으로 대체되어야 합니다. 속성은 검색하려는 속성 열의 이름이며, '값'은 원하는 값으로 대체되어야 합니다.
				</li>
				</br>
				<li>6. <strong>레이어 속성 정렬</strong>: <code>SELECT * FROM 레이어_테이블명 ORDER BY 속성 ASC/DESC;</code>
				</br>위의 쿼리문은 특정 속성 열을 기준으로 레코드를 오름차순(ASC) 또는 내림차순(DESC)으로 정렬하여 조회하는 예시입니다.
				</li>
				</br>
				<li>7. <strong>레이어 속성 그룹화</strong>: <code>SELECT 속성, COUNT(*) FROM 레이어_테이블명 GROUP BY 속성;</code>
				</br>위의 쿼리문은 특정 속성 열을 기준으로 레코드를 그룹화하고, 각 그룹에 대한 레코드 수를 계산하여 조회하는 예시입니다.
				</li>
				</br>
				<li>8. <strong>레이어 조인</strong>: <code>SELECT * FROM 레이어_테이블1 INNER JOIN 레이어_테이블2 ON 조인_조건;</code>
				</br>위의 쿼리문은 두 개의 레이어 테이블을 조인하여 관련된 정보를 가져오는 예시입니다. 레이어_테이블1과 레이어_테이블2는 실제 레이어 테이블의 이름으로 대체되어야 합니다. 조인_조건은 두 테이블 간의 관계를 정의하는 조인 조건으로 대체되어야 합니다.
				</li>
			</ul>
		</div>
</div>
		...
		<ul>
			<li>
				<div class="imgBox">
					...
					<p style="margin-bottom:2px;">1. eGovFrame을 실행시켜서 <strong>SVN Repositories</strong>에서 <strong>New > Repository Location</strong>을 클릭합니다.</p>

					<img src="/images/svn-help/image2.png">
					<p style="margin-bottom:2px;">2. URL 칸에 <strong>svn://seesunit.iptime.org:36901/projects</strong> 입력하고, User 칸에 <strong>자신의 이름</strong>을 입력하고 <strong>Finish</strong> 클릭합니다. </br>(Password가 있는 경우 비번 입력)</p>
					
					<img src="/images/svn-help/image3.png">
					<p style="margin-bottom:2px;">3. <strong>SVN Repositories</strong> 상자 안에 이미지와 같이 연결됩니다.</p>
					
					<img src="/images/svn-help/image4.png">
					<p style="margin-bottom:2px;">4. <strong>commit</strong>하고 싶은 프로젝트를 우클릭해서 <strong>Team > Apply Patch...</strong>을 클릭합니다.</p>
					
					<img src="/images/svn-help/image5.png">
					<p style="margin-bottom:2px;">5. 이미지와 같이 보이면<strong> Finish</strong>을 클릭합니다.</p>
					
					<img src="/images/svn-help/image6.png">
					<p style="margin-bottom:2px;">6. 연결한 프로젝트를 우클릭 하고<strong> Team > Synchronize with Repository</strong>를 클릭합니다.</p>
					
					<img src="/images/svn-help/image7.png">
					<p style="margin-bottom:2px;">7. <strong>Comment</strong>를 작성하고 <strong>commit</strong> 할 폴더 나 파일을 선택한 후, <strong>Ok</strong>를 클릭합니다.</p>
					
					<img src="/images/svn-help/image8.png">
					<p style="margin-bottom:2px;">8. 이미지와 같이 업로드 되면 <strong>성공</strong>입니다.</p>
				</div>
			</li>
		</ul>
		</div>
